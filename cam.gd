...
var _mouse_abs_position := Vector2.ZERO
...
func _unhandled_input(event):
	...
	if event is InputEventMouseMotion:
		...
		_mouse_abs_position = event.position
...
func _update_mouselook():
	...
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		if _ctrl:
			position += (transform.basis.y.normalized() / 90.0) * pitch
			position -= (transform.basis.x.normalized() / 90.0) * yaw
		else:
